...
if (!BLOG_SHEET_ID || BLOG_SHEET_ID === 'YOUR_BLOG_GOOGLE_SHEET_ID_HERE') {
  console.warn('⚠️  GOOGLE_BLOG_SHEET_ID is not configured — skipping blog fetch');
  console.warn('   Add GOOGLE_BLOG_SHEET_ID to your .env file to enable this.');
  process.exit(0);
}
...
async function fetchSheetRange(range) {
  try {
    ...
    if (error.message.includes('Unable to parse range') || error.message.includes('notFound')) {
      console.error(`❌ Tab "${tabName}" not found in your Blog Google Sheet.`);
      console.error(`   Open your sheet and make sure a tab is named exactly: ${tabName}`);
      console.error(`   Tab names are case-sensitive. Current config: SHEETS.BLOG_POSTS = '${SHEETS.BLOG_POSTS}'`);
      console.error(`   To fix: rename the tab in Google Sheets, or update SHEETS.BLOG_POSTS in scripts/sheets-config.js`);
    } else {
...
async function fetchAndGenerateBlog() {
  ...
  try {
    ...
    console.warn('⚠️  Blog fetch failed — using existing src/data/blog.js if it exists.');
    console.warn('   Fix the issue above, then re-run: npm run fetch-blog');
    process.exit(0);
  }
}
